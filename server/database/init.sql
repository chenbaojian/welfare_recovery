...
-- 创建数据库
CREATE DATABASE IF NOT EXISTS welfare_recovery DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `admin` (
  ...
  `real_name` VARCHAR(20) DEFAULT NULL COMMENT '真实姓名',
  `role` VARCHAR(20) DEFAULT 'OPERATOR' COMMENT '角色: SUPER_ADMIN-超级管理员, ADMIN-管理员, OPERATOR-运营',
  `status` VARCHAR(10) DEFAULT 'ACTIVE' COMMENT '状态: ACTIVE-正常, DISABLED-禁用',
...
CREATE TABLE IF NOT EXISTS `balance_log` (
  ...
  `type` VARCHAR(20) NOT NULL COMMENT '类型: RECYCLE_INCOME-回收收入, WITHDRAW-提现支出, ADMIN_ADJUST-管理员调整',
  `amount` DECIMAL(10,2) NOT NULL COMMENT '变动金额(正数增加,负数减少)',
  `balance_before` DECIMAL(10,2) NOT NULL COMMENT '变动前余额',
  `balance_after` DECIMAL(10,2) NOT NULL COMMENT '变动后余额',
  `remark` VARCHAR(255) DEFAULT NULL COMMENT '备注',
  `operator_id` INT DEFAULT NULL COMMENT '操作人ID(管理员调整时)',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
  KEY `idx_user_id` (`user_id`),
